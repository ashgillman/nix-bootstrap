...
function mmi () {
	make -j $NUM_THREADS 2&>1 1>/dev/null
...
mmi
popd
# TODO add detection for when gcc is new enough (gcc49 is minimal)
pushd gcc-4.9.2
...
mkdir -p gcc-objs
pushd gcc-objs
$PWD/../gcc-4.9.2/configure --prefix=$MYTMP --enable-languages=c,c++
mmi
popd
...
popd
ln -s $HOME/nix/var/nix/profiles/default ~/.nix-profile
